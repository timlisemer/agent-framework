...
| Path | What's there |
|------|--------------|
| `~/.claude/projects/<encoded>/<session-id>.jsonl` | Raw Claude transcript — every user/assistant/tool_result line, the user's literal text, the assistant's literal text, tool inputs and outputs. |
...
| `~/.agent-framework/sessions/<encoded>/<dir>/transcript-path.txt` | Sidecar: absolute path back to the corresponding raw adapter transcript. The materializer infers the adapter from this path when it contains `/.claude/` or `/.codex/`. |
...
rg -n --no-heading --color=never "<QUOTE>" ~/.claude/projects/ ~/.codex/sessions/ | head -30
```
...
For Codex transcripts there may be no raw `uuid`; use the hit's file path and
nearby transcript ordering instead, then choose the closest capture as described
below.
...
# Prefer this MCP action over ad-hoc node -e. It works when shell execution is
# blocked and writes a durable scenario under ~/.agent-framework/test-runs/scenarios/.
mcp__agent-framework__scenario_tester({
  "action": "materialize_scenario",
  "session_dir": "$SESSION_DIR",
  "capture_seq": <CAPTURE_SEQ>,
  "scenario_name": "<optional-slug>",
...
The session is too old and was rotated out of the capture cap (default 5000 entries per session, configurable via `AGENT_FRAMEWORK_CAPTURE_CAP`). The transcript JSONL may still exist under the adapter's transcript directory, such as `~/.claude/projects/` or `~/.codex/sessions/`; you can read it directly without a paired capture.
...
1. Materialize it via `mcp__agent-framework__scenario_tester` action `materialize_scenario`.
2. Use `"run_materialized": true` on the materialize call, or run it afterward via action `run_scenario`, to confirm it reproduces.
...
- The `scenario_tester` MCP tool's `list_scenarios` action only enumerates stored scenario fixtures. It does NOT walk live captures under `~/.agent-framework/sessions/`. For the recipe above, raw filesystem grep + `materialize_scenario` is the path; if cross-session search becomes a regular need, a future `scenario_find` MCP tool would be the right place to encapsulate it.
