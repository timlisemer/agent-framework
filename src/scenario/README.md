...
- `plan-validation-status.json` caches exact-content plan validation results,
  ...
  MCP calls write this cache; Codex Stop plan-exit hooks use it to trust exact
  populated planfile content that has already passed validation. Populated
  planfiles are the source of truth and are not overwritten from inline
  transcript text; missing or empty planfiles keep the extracted content for
  remediation.
