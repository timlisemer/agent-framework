...
import { RECENT_MESSAGES_RULE_POLICY } from "./policies.js";
...
export const recentMessagesRule: PreToolRule = {
  ...
  version: "1",
  configuration: RECENT_MESSAGES_RULE_POLICY,
  ...
  async check(ctx: RuleContext): Promise<RuleCheckResult> {
    const oldestFirst =
      ctx.recentUserMessages && ctx.recentUserMessages.length > 0
        ? ctx.recentUserMessages.map((content, index) => ({
            role: "user" as const,
            content,
            index,
          }))
        : ((
            await readTranscriptExact(ctx.transcriptPath, {
              counts: {
                user: {
                  count: RECENT_MESSAGES_RULE_POLICY.transcriptUserMessageCount,
                },
              },
          excludeSlashCommandPrompts: true,
            ...
            .slice()
            .reverse() ?? []);
    if (oldestFirst.length < RECENT_MESSAGES_RULE_POLICY.minimumMessages)
      return null;
    const block =
