...
import { isHostileContext, detectStallShape } from "../utils/stall-detect.js";
...
import {
  RESPONSE_ALIGN_STOP_RULE_POLICY,
} from "./policies.js";
import { observableAgentConfiguration } from "./policy-observability.js";
...
const CLASSIFY_STOP_RESPONSE_AGENT_CONFIG: Omit<AgentConfig, "workingDir"> = {
  ...
  tier: RESPONSE_ALIGN_STOP_RULE_POLICY.classifierModelTier,
  mode: "direct",
  maxTokens: RESPONSE_ALIGN_STOP_RULE_POLICY.classifierMaximumTokens,
...
export const responseAlignStopRule: PreToolRule = {
  ...
  configuration: {
    ...RESPONSE_ALIGN_STOP_RULE_POLICY,
    evaluationAgents: {
      classifier: observableAgentConfiguration(
        CLASSIFY_STOP_RESPONSE_AGENT_CONFIG,
      ),
      questionVerifier: observableAgentConfiguration(
        VERIFY_USER_QUESTION_AGENT_CONFIG,
      ),
    },
  },
  ...
  async check(ctx: RuleContext): Promise<RuleCheckResult> {
    ...
    if (
      priorErrorEvaluation.status === "satisfied" &&
      priorErrorEvaluation.fullPass === true
    ) {
      return null;
    }
    ...
    if (stallReason) {
      ...
      const isAnnouncementWithoutActionStall =
        stallReason ===
        "announcement-without-action repeating just-rejected promise-stop";
    ...
    const isShortResponse =
      trimmedAssistant.length <
        RESPONSE_ALIGN_STOP_RULE_POLICY.shortResponseCharacters &&
    ...
    if (userQuestion) {
      ...
      if (
        strippedAssistant.length <
        RESPONSE_ALIGN_STOP_RULE_POLICY.answerEvidenceCharacters
      ) {
        if (
          /^(?:I'll|Let me|Sure|OK|Okay|Got it|Understood)/.test(assistantText)
        ) {
          ...
          const isQuestion =
            verifyTrimmed.includes("QUESTION") &&
            !verifyTrimmed.includes("NOT_QUESTION");

          if (isQuestion) {
            return {
              stopBlock: `${USER_QUESTION_NOT_ANSWERED_SYSTEM_MESSAGE_PREFIX}${userQuestion}"\nPlease respond to what they asked.`,
            };
          }
        }
      }
