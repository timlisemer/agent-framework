...
const PLANFILE_EDIT_REFUSAL_SYSTEM_MESSAGE =
...
export const responseAlignStopRule: PreToolRule = {
  ...
  async check(ctx: RuleContext): Promise<RuleCheckResult> {
    ...
    const planfileEditRefusalReason = ctx.planMode
      ? detectPlanfileEditRefusal(
        assistantText,
        [userText, ctx.planModeCtx.contextString, ...(ctx.recentUserMessages ?? [])].join("\n"),
      )
      : null;
    if (planfileEditRefusalReason) {
      return { stopBlock: PLANFILE_EDIT_REFUSAL_SYSTEM_MESSAGE };
    }
    if (
      ctx.planMode &&
      isPlanfileDirectEditCommitment(
    ...
    // We need to check the transcript for stop hook error since we don't have it directly.
    // Build a stop hook error indicator from the userText itself (the injected message appears
    // in user content in the transcript, which is already in ctx.userText / ctx.assistantText).
    const stopHookErrorInContext = stopHookErrorPattern.test(userText);

    const needsLLMCheck =
      questionCheck.detected ||
      stopHookErrorInContext ||
    ...
    if (needsLLMCheck) {
      ...
      const stopHookSection = stopHookErrorInContext
