...
- Include an Assistant Verification section that states the agent-framework check MCP wire name is run with repository working_dir after each larger code change as the repository-level replacement for language-specific shell check and test commands. It must not tell the assistant to run targeted shell tests or package-manager test commands before the MCP.
