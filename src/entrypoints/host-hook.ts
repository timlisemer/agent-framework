...
import type { AdapterEncoder, EncodedOutput, PlanModeDetection } from "../adapter/types.js";
...
import { FILE_TOOLS, extractFilePaths, isPathInDirectory, isPlanFile } from "../rules/utils.js";
...
import { isTerminalToolStatus, scenarioToolStatusSchema } from "../scenario/protocol/snapshot.js";
...
export function createHostHookStartCommand(
  ...
  return createHostHookScenarioCommand(context, {
    type: "startRun",
...
export async function dispatchPreToolUse(
  ...
  return encoder.encodePreToolUseDeny(result.reason ?? "Scenario runtime denied the tool call");
}
...
export async function dispatchPreToolUseResult(
  ...
  const canonical = boundary.spec.canonicalizeToolCall(input.tool_name, input.tool_input);
  const state = await workflowState(boundary);
  const context = await preToolContext(input, boundary, canonical.toolName, canonical.toolInput);
...
export async function dispatchUserPromptSubmit(
  ...
  const state = await workflowState(boundary);
  const workflowInvocation = boundary.spec.recognizeWorkflowInvocation(input.prompt);
  const context: HostRuntimeContext = {
    ...
    userPrompt: {
      ...
      workflowInstructionText: workflowInvocation
        ? boundary.spec.workflowInstructionText(workflowInvocation, boundary.host)
        : null,
      workflowOnly: Boolean(workflowInvocation && boundary.spec.isWorkflowInvocationOnly(input.prompt)),
      planExit: boundary.spec.isPlanExit({ event: "UserPromptSubmit", prompt: input.prompt }),
    },
  };
  ...
  if (result.status === "denied") {
    const reason = result.reason ?? "User prompt was blocked by the scenario runtime";
...
export async function dispatchStop(
  ...
  const planExitText = [input.last_assistant_message ?? null, ...assistantTextCandidates]
    .find((candidate) => boundary.spec.isPlanExit({ event: "Stop", assistantText: candidate })) ?? null;
  const context: HostRuntimeContext = {
    ...baseHostContext(input, boundary),
    stop: {
      ...
      priorErrorContext: transcript.priorErrorContext
        .map((entry) => ({ ...entry, provenance: [...entry.provenance] })),
      planExitText,
      stopBlockDisabled: process.env.AGENT_FRAMEWORK_DISABLE_STOP_BLOCK === "1" &&
  ...
  if (result.status === "failed") throw new Error(result.reason ?? "Stop runtime effect failed");
  return result.status === "stopBlocked"
    ? encoder.encodeStopBlock(result.reason ?? "Stop was blocked by the scenario runtime")
...
export async function dispatchSessionStart(
  ...
  const boundary = await openHookBoundary(input, options);
  const workflow = input.source === "compact" || input.source === "clear"
  ...
  if (result.status === "failed") throw new Error(result.reason ?? "SessionStart runtime command failed");
...
async function dispatchPostToolOutcome(
  ...
  const continuation = outcome === "completed"
    ? boundary.spec.continuationAfterToolResult(
        canonical,
        "tool_response" in input ? input.tool_response : undefined,
      )
    : boundary.spec.continuationAfterToolFailure(canonical, error ?? "", outcome === "cancelled");
  ...
  const workflow = requirement
    ...
    : initialState;
  const currentPlan = outcome === "completed"
    ? await currentPlanAfterTool(boundary.sessionDir, canonical.toolName, canonical.toolInput)
    : undefined;
  const canonicalInput = jsonValue(canonical.toolInput);
  const inputDigest = digestScenarioValue(canonicalInput);
  const toolCallId = input.tool_use_id ?? boundary.postToolCallId(canonical.toolName, inputDigest);
  const context: HostRuntimeContext = {
    ...baseHostContext(input, boundary),
    postTool: {
  ...
  const result = await boundary.dispatch({
    ...
    ...(outcome === "completed" && "tool_response" in input
      ...
      : {}),
    error,
    ...(currentPlan === undefined ? {} : { currentPlan: jsonValue(currentPlan) }),
  });
  if (result.status === "failed") throw new Error(result.reason ?? "PostToolUse runtime command failed");
  return encoder.encodeOk(outcome === "completed" ? "PostToolUse" : "PostToolUseFailure");
}
...
async function openHookBoundary(
  ...
  dispatch(payload: AgentFrameworkHostCommand): Promise<Awaited<ReturnType<ScenarioRuntime["dispatch"]>>>;
}> {
  ...
  const runId = canonicalHookRunId(spec.name, input.transcript_path);
  const runtime = options.runtime ?? createAgentFrameworkScenarioRuntime();
  let workflowBaseline: { state: SessionWorkflowState; revision: number } | null = null;
  ...
  const command = (
    ...
    expectedSnapshotRevision?: number,
    commandId?: string,
  ): ScenarioCommand => createHostHookScenarioCommand(commandContext, payload, {
  ...
  const inputPrompt = "prompt" in input && typeof input.prompt === "string" ? input.prompt : null;
  const promptDigest = inputPrompt === null ? null : digestScenarioValue(inputPrompt);
  const providedPromptDeliveryId = inputPrompt === null ? null : input.delivery_id ?? null;
  if (providedPromptDeliveryId !== null && providedPromptDeliveryId.length === 0) {
    throw new Error("UserPromptSubmit delivery_id must not be empty");
  }
  let importedView: Awaited<ReturnType<ScenarioRuntime["canonicalView"]>> | null = null;
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    const existingWorkflow = sessionWorkflowStateFromJson(
      existingSnapshot.stateSlices["session.workflow"]?.value,
    );
    const cachedUserMessage = existingWorkflow.currentPrediction?.userMessageFull ??
      existingWorkflow.currentPrediction?.userMessageSnippet ?? "";
    const toolUseId = "tool_use_id" in input && typeof input.tool_use_id === "string"
      ? input.tool_use_id
      : undefined;
    const nativeTranscript = await canonicalNativeTranscriptObservation({
    ...
    const nativeData = nativeTranscript.data as Record<string, JsonValue>;
    if (Array.isArray(nativeData.tools)) {
      nativeData.tools = nativeData.tools.filter((tool) => {
        if (!isRecord(tool)) return false;
        if ("tool_use_id" in input && tool.id === input.tool_use_id) return false;
        const status = scenarioToolStatusSchema.safeParse(tool.status);
        return status.success && isTerminalToolStatus(status.data);
      });
    }
    const priorNativeState = existingSnapshot.stateSlices["transcript.native"]?.value;
    const activeNativeProjection = isRecord(priorNativeState) && (
      (Array.isArray(priorNativeState.messageIds) && priorNativeState.messageIds.length > 0) ||
      (Array.isArray(priorNativeState.toolCallIds) && priorNativeState.toolCallIds.length > 0)
    );
    if (nativeTranscript.availability === "missing") {
      if (activeNativeProjection) {
        throw new Error("Native transcript is unavailable while canonical native history is active");
      }
      const currentView = await runtime.canonicalView(runId);
      if (currentView.snapshot.revision !== existingSnapshot.revision) continue;
      const committedWorkflow = sessionWorkflowStateFromJson(
        currentView.snapshot.stateSlices["session.workflow"]?.value,
      );
      const committedCachedUserMessage = committedWorkflow.currentPrediction?.userMessageFull ??
        committedWorkflow.currentPrediction?.userMessageSnippet ?? "";
      if (committedCachedUserMessage !== cachedUserMessage) continue;
      importedView = currentView;
      importedMetadata = nativeTranscript.metadata;
      observedPromptMessageId = undefined;
      break;
    }
    const priorNativeMessageIds = isRecord(priorNativeState) && Array.isArray(priorNativeState.messageIds)
      ? new Set(priorNativeState.messageIds.filter((id): id is string => typeof id === "string"))
      : new Set<string>();
    const priorNativePromptOccurrences = promptDigest === null
      ? 0
      : existingSnapshot.conversation.filter((message) =>
          priorNativeMessageIds.has(message.id) &&
          message.role === "user" &&
          message.contentDigest === promptDigest
        ).length;
    const nativePromptOccurrences = promptDigest === null || !Array.isArray(nativeData.messages)
      ? []
      : nativeData.messages.filter((message): message is Record<string, JsonValue> =>
          isRecord(message) &&
          message.role === "user" && message.contentDigest === promptDigest && message.content === inputPrompt
        );
    const observedPromptMessageIdCandidate = nativePromptOccurrences.at(-1)?.id;
    const nativeDigest = digestScenarioValue(nativeData);
    const priorImport = existingSnapshot.stateSlices["transcript.native"]?.value;
    if (!isRecord(priorImport) || priorImport.digest !== nativeDigest) {
      try {
        await runtime.dispatch(command({
          type: "nativeTranscriptObserved",
          data: nativeTranscriptDataSchema.parse(jsonValue({
            ...nativeData,
            digest: nativeDigest,
          })),
        }, existingSnapshot.revision));
      } catch (error) {
        if (error instanceof SnapshotRevisionConflictError) continue;
        throw error;
      }
    }
    importedView = await runtime.canonicalView(runId);
    const committedNativeState = importedView.snapshot.stateSlices["transcript.native"]?.value;
    if (!isRecord(committedNativeState) || committedNativeState.digest !== nativeDigest) {
      importedView = null;
      continue;
    }
    const committedWorkflow = sessionWorkflowStateFromJson(
      importedView.snapshot.stateSlices["session.workflow"]?.value,
    );
    const committedCachedUserMessage = committedWorkflow.currentPrediction?.userMessageFull ??
      committedWorkflow.currentPrediction?.userMessageSnippet ?? "";
    ...
    importedMetadata = nativeTranscript.metadata;
    const candidateIsActive = typeof observedPromptMessageIdCandidate === "string" &&
      importedView.snapshot.conversation.some((message) =>
        message.id === observedPromptMessageIdCandidate &&
        message.role === "user" &&
        message.contentDigest === promptDigest
      );
    const candidateWasSubmitted = candidateIsActive && importedView.records.some((record) =>
      ...
      record.payload.messageId === observedPromptMessageIdCandidate
    );
    observedPromptMessageId = candidateIsActive && (
        nativePromptOccurrences.length !== priorNativePromptOccurrences || !candidateWasSubmitted
      )
      ? observedPromptMessageIdCandidate
      : undefined;
    break;
  }
  if (!importedView || !importedMetadata) {
    throw new Error("Native transcript import did not converge after repeated revision conflicts");
  }
  const importedSnapshot = importedView.snapshot;
  const committedNativeState = importedSnapshot.stateSlices["transcript.native"]?.value;
  const nativeTranscriptIdentity = isRecord(committedNativeState) &&
      typeof committedNativeState.digest === "string"
    ? committedNativeState.digest
    : digestScenarioValue({ availability: "missing", runId });
  const promptOccurrence = inputPrompt === null
    ? null
  ...
  const promptCommandId = promptOccurrence === null
    ? null
    : hostBoundaryCommandId("hostUserPromptSubmitted", promptOccurrence);
  if (providedPromptDeliveryId !== null && promptCommandId !== null) {
    const accepted = importedView.records.find((record) =>
      record.commandId === promptCommandId && record.eventType === "command.accepted"
    );
    if (accepted) {
      const storedCommand = scenarioCommandSchema.parse(accepted.payload.command);
      const storedHostCommand = agentFrameworkHostCommandData(storedCommand.payload);
  ...
  workflowBaseline = {
    state: sessionWorkflowStateFromJson(importedSnapshot.stateSlices["session.workflow"]?.value),
  ...
  const boundaryCommandId = (payload: AgentFrameworkHostCommand): string => {
    const occurrence = payload.type === "hostPreToolUse" || payload.type === "hostPostToolUse"
      ? { type: payload.type, toolCallId: payload.toolCallId }
      : payload.type === "hostUserPromptSubmitted"
        ? null
        : payload.type === "hostSessionStarted"
          ? { type: payload.type, nativeTranscriptIdentity, source: payload.source }
          : { type: payload.type, nativeTranscriptIdentity };
    if (payload.type === "hostUserPromptSubmitted") {
      if (promptCommandId === null) throw new Error("UserPromptSubmit occurrence identity is unavailable");
  ...
  const dispatchCanonical = async (
    ...
    const currentView = await runtime.canonicalView(runId);
    const accepted = currentView.records.find((record) =>
      record.commandId === commandId && record.eventType === "command.accepted"
    );
    if (accepted) {
      const storedCommand = scenarioCommandSchema.parse(accepted.payload.command);
      const storedHostCommand = agentFrameworkHostCommandData(storedCommand.payload);
  ...
  const dispatch = async (payload: AgentFrameworkHostCommand) => {
    const reconciledPayload = payload.type === "hostUserPromptSubmitted" &&
        typeof observedPromptMessageId === "string" && payload.contentDigest === promptDigest
      ? { ...payload, observedMessageId: observedPromptMessageId }
      : payload;
    const workflowPayload = hasWorkflowPayload(reconciledPayload) ? reconciledPayload : null;
    ...
    return dispatchAgentFrameworkWorkflow({
      ...
      dispatch: (workflow, expectedSnapshotRevision) => dispatchCanonical(
        {
          ...workflowPayload,
          workflow: jsonValue(workflow),
  ...
  return {
    ...
    postToolCallId: (name, inputDigest) => {
      const matching = importedSnapshot.toolCalls.filter((tool) =>
        tool.name === name && tool.inputDigest === inputDigest
      );
      const nonterminal = matching.filter((tool) => !isTerminalToolStatus(tool.status));
      const canonical = nonterminal.length === 1
        ? nonterminal[0]
        : nonterminal.length === 0 ? matching.at(-1) : undefined;
      return canonical?.id ??
...
function hostBoundaryCommandId(type: AgentFrameworkHostCommand["type"], occurrence: unknown): string {
...
function baseHostContext(input: BaseHookInput, boundary: HookBoundary): HostRuntimeContext {
  return {
...
async function preToolContext(
  ...
  if (FILE_TOOLS.includes(toolName)) {
    for (const raw of extractFilePaths(toolName, toolInput)) {
      const absolute = path.isAbsolute(raw) ? raw : path.resolve(boundary.host.projectDir, raw);
      if (!isPathInDirectory(absolute, boundary.host.projectDir) && !isPlanFile(absolute, boundary.sessionDir)) {
  ...
  const cachedSnippetSideTaskDischarged = boundary.transcriptMetadata.cachedSnippetSideTaskDischarged;
  const slashCommandAllowedTools = boundary.transcriptMetadata.slashCommandAllowedTools;
  ...
  const batch = batchInfo
    ? {
        ...
        calls: batchInfo.members.map((member) => {
          const memberCanonical = member.toolUseId === input.tool_use_id
            ? { toolName, toolInput }
            : boundary.spec.canonicalizeToolCall(member.toolName, member.toolInput);
          return {
            toolCallId: member.toolUseId,
            name: memberCanonical.toolName,
            input: jsonValue(memberCanonical.toolInput),
            mayRequireContinuation: boundary.spec.toolResultMayRequireContinuation(memberCanonical),
          };
        }),
      }
    : null;
...
async function workflowState(boundary: HookBoundary): Promise<SessionWorkflowState> {
