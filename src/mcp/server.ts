...
server.registerTool(
  "test_harness_tester",
  {
    ...
    inputSchema: {
      ...
      scenario_name: z.string().optional().describe("For run_scenario / read_scenario: slug identifying a scenario. For run_scenario, resolved across the union of four sources: ~/.agent-framework/test-runs/scenarios/<name>/scenario.json (home) and <AGENT_FRAMEWORK_ROOT>/test-harness/fixtures/scenarios/{working,broken,todo}/<name>.json (committed fixtures). Slugs must be unique across all four sources. read_scenario still reads only from the home tree. Must match [A-Za-z0-9._-]+."),
      ...
      scenario_names: z.array(z.string()).optional().describe("For run_scenarios: explicit list of scenario slugs to run. Resolved against the UNION of four sources: ~/.agent-framework/test-runs/scenarios/<name>/scenario.json (home) and <AGENT_FRAMEWORK_ROOT>/test-harness/fixtures/scenarios/{working,broken,todo}/<name>.json (committed fixtures). A slug present in two or more sources is a hard error. Fixtures run in place; reports + cache always land under the home tree. Omit or pass an empty array to run EVERY scenario in the union, alphabetically. Returns aggregated JSON {total, passed, failed, results[]} with per-result {source: \"home\"|\"fixture-working\"|\"fixture-broken\"|\"fixture-todo\"}. Each result also carries `expectation_reality: \"working\" | \"broken\"` and `expectation_reality_last_run_at: ISO-8601` — the most recent run's reflection, written back to the scenario source file. A mismatch between folder and reality (e.g. reality=\"broken\" for a fixture in working/) surfaces regressions; the inverse surfaces landed features or fixed bugs. The aggregate response does NOT summarize mismatches; callers inspect `results[]` directly."),
      scenario_source: z.enum(["working", "broken", "todo", "home"]).optional().describe("For run_scenarios / list_scenarios: scope to ONE source. 'working' | 'broken' | 'todo' select fixture subfolders (<AGENT_FRAMEWORK_ROOT>/test-harness/fixtures/scenarios/<sub>/). 'home' selects ~/.agent-framework/test-runs/scenarios/. Omit to include all four. Slug uniqueness is enforced across all four roots regardless of filter."),
    }
  },
  async (args) => {
