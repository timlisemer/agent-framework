...
server.registerTool(
  "test_harness_tester",
  {
    ...
    inputSchema: {
      ...
      scenario_name: z.string().optional().describe("For run_scenario / read_scenario: slug identifying a scenario. For run_scenario, resolved across the union of ~/.agent-framework/test-runs/scenarios/<name>/scenario.json (home) and <AGENT_FRAMEWORK_ROOT>/test-harness/fixtures/scenarios/<name>.json (committed fixture). Slugs must be unique across sources. read_scenario still reads only from the home tree. Must match [A-Za-z0-9._-]+."),
      ...
      scenario_names: z.array(z.string()).optional().describe("For run_scenarios: explicit list of scenario slugs to run. Resolved against the UNION of ~/.agent-framework/test-runs/scenarios/<name>/scenario.json and <AGENT_FRAMEWORK_ROOT>/test-harness/fixtures/scenarios/<name>.json. A slug present in both trees is a hard error (slugs must be unique across sources). Fixtures run in place; reports + cache always land under the home tree. Omit or pass an empty array to run EVERY scenario in the union, alphabetically. Returns aggregated JSON {total, passed, failed, results[]} with per-result {source: \"home\"|\"fixture\"}."),
    }
  },
  async (args) => {
