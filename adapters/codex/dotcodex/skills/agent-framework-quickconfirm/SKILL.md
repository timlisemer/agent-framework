...
If the confirm result is `DECLINED` or contains errors, spell out the specific errors returned by the MCP. If multiple errors are returned, list the multiple errors; do not replace them with only a vague count or summary such as `check failed with N errors`.
