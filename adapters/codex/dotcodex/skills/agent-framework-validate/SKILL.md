---
name: agent-framework-validate
description: Validate that a plan was implemented correctly through the agent-framework validate_implementation MCP. Use when the user invokes $agent-framework-validate.
---
...
Immediately call `mcp__agent_framework__validate_implementation`.
...
Do not validate the implementation yourself before calling the MCP. Do not use `validate_plan`; that is only for plan-contract validation. Report the MCP result.
