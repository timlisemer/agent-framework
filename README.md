...
plan-state, and cancel frames; the backend emits
session, turn, message, resume-metadata, plan-state, and error events using
the generated types exported from `src/ai-protocol`.

AI protocol bindings are generated by Astral and copied into this repository
with `npm run sync:ai-protocol`. Set `ASTRAL_ROOT` or pass
`--astral-root /path/to/astral` when the Astral checkout is not in one of the
default sibling locations. Build before running the backend:
