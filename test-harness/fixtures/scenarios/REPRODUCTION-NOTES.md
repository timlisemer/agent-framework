...
7. **Commit the scenario as a fixture, or iterate locally inline.** To add a permanent regression scenario, write `test-harness/fixtures/scenarios/{working,broken,todo}/<name>.json` (filename stem == inner `scenario.name`). Pick the subfolder per the rules in each folder's `README.md`: `working/` = passes today, `broken/` = fixture itself is wrong, `todo/` = description contains "EXPECTED TO FAIL" (feature not yet implemented). `run_scenarios`, `run_scenario`, and `list_scenarios` read fixtures directly from the repo — edits are picked up on the very next run, no cache sync step. For short-lived experiments, pass the full `scenario` object inline to `run_scenario`; it writes to `~/.agent-framework/test-runs/scenarios/<name>/scenario.json` and future by-name calls resolve it from there. A slug that exists in more than one of the four sources (`~/.agent-framework/test-runs/scenarios/<name>/` + `test-harness/fixtures/scenarios/{working,broken,todo}/<name>.json`) is a hard error listing every offending path — delete all but one copy.
...
5. Fixture edits not reflected → fixtures are read directly from `test-harness/fixtures/scenarios/{working,broken,todo}/<name>.json` on every run. If you see stale behavior, make sure you saved the file and that no duplicate copy exists under another fixture subfolder or at `~/.agent-framework/test-runs/scenarios/<name>/scenario.json` (a slug present in two or more of the four sources triggers a cross-source collision error, not silent staleness).
