...
Curated real JSONL transcripts for hook replay testing.
...
3. **Run `--list` on the full transcript first** to identify which tool calls you want to test:
   ```bash
   npx tsx test-harness/run.ts --list test-harness/fixtures/transcripts/<your-file>.jsonl
   ```
   Note the line numbers of interest. Then trim the file to only include lines 0 through the highest line number you need. A typical fixture is 10-30 lines.
4. **Sanitize the transcript** by editing the JSONL file in a text editor. Individual lines can be long (1000+ characters) — search-and-replace is more practical than eyeballing. For each line:
...
### `git-blame-investigation.jsonl`

A 16-line session where the user asks to find the commit that introduced an error message. Contains 4 tool calls:

| Line | Tool | Expected | Agent | Description |
|---|---|---|---|---|
| 6 | Grep | allow | low-risk-bypass | Search for error string in codebase |
| 8 | Read | allow | low-risk-bypass | Read source file at specific offset |
| 11 | Bash | allow | — | `git blame` on source file |
| 14 | Bash | allow | — | `git log` for commit message |
