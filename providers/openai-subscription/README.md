...
- Internal write runtime sessions use per-run homes under `~/.agent-framework/internal/write/codex/<runId>` with the same configured adapter surface as the generic managed profile, including MCP servers, plugins, project trust, and file editing tools, while removing only the Stop hook. Those disposable runtime homes are removed when the run ends, and framework implementation session state persists under `~/.agent-framework/internal/sessions/write/<runId>`.
...
- In generic managed user-runtime sessions (`{ kind: "managed", configuration: { profile: "default" } }`), agent-framework sets `CODEX_HOME` to `~/.agent-framework/managed/default/codex`, refreshes framework-owned adapter config there, and preserves `sessions/` history plus top-level auth/local-secret files.
