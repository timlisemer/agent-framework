...
function latestUserTurn(rawText: string, logicText = rawText): LatestUserTurn {
  return {
    rawText,
    logicText,
    displaySnippet: logicText.slice(0, 200),
...
describe("step 3.7 path (a'): class-level fresh-imperative re-authorization", () => {
  ...
  it("'now implement' + Edit with stale blockAllTools=true -> allow (latest live turn wins)", () => {
    const pred = makeAngryLowStreak4({
      blockAllTools: true,
    ...
    const result = decidePrediction(
      ...
      false,
      [],
      latestUserTurn("now implement"),
    );
    expect(result.decision).toBe("allow");
    expect(result.reason).toContain("Latest user message authorizes Edit");
  });
  ...
  it("quoted code plus latest undo text uses latest display snippet and authorizes Edit", () => {
    const pred = makeAngryLowStreak4({
      userMessageSnippet: "\"const PLAN_APPROVAL_SYSTEM_MESSAGE =",
      intent: "The user wants removed stop-hook feedback restored.",
    });
    const raw =
      "\"const PLAN_APPROVAL_SYSTEM_MESSAGE =\\n  \\\"[AUTOGENERATED STOP HOOK FEEDBACK]\\\";\\n\"\\n\\nyou removed that error message and probably the logic behind it aswell. this was not requested and needs to be undone";
    const logic =
      "you removed that error message and probably the logic behind it aswell. this was not requested and needs to be undone";
    const result = decidePrediction(
      pred,
      "Edit",
      { file_path: "src/rules/response-align-stop.ts" },
      1,
      raw,
      [],
      false,
      [],
      latestUserTurn(raw, logic),
    );
    expect(result.decision).toBe("allow");
    expect(result.reason).toContain("Latest user message authorizes Edit");
  });

  it("latest fix/remove/reuse text authorizes read-only Bash support despite cached anger", () => {
    const pred = makeAngryLowStreak4({
      userMessageSnippet: "\" - Move/reuse\" dont you dare to move the duplicate code",
      intent: "The user wants duplicate code removed and existing code reused.",
    });
    const logic =
      "dont you dare to move the duplicate code into a different location now remove the duplicate code and reuse the existing code";
    const result = decidePrediction(
      pred,
      "Bash",
      ...
      false,
      [],
      latestUserTurn(`" - Move/reuse" ${logic}`, logic),
    );
    expect(result.decision).toBe("allow");
    expect(result.reason).toContain("read-only Bash inspection supports that live request");
  });

  it("quoted Move text does not authorize a move command", () => {
    const pred = makeAngryLowStreak4({
      userMessageSnippet: "\" - Move/reuse\" dont you dare to move the duplicate code",
      intent: "The user wants duplicate code removed and existing code reused.",
    });
    ...
    const result = decidePrediction(
      pred,
      "Bash",
      { command: "mv src/utils/markdown-plan.ts src/utils/content-patterns.ts" },
      3,
      logic,
      [],
      false,
  ...
  it("latest no-tools text denies even when cached prediction authorizes edit", () => {
    const pred = makeAngryLowStreak4({
      explicitlyAllowedTools: ["Edit"],
      userMessageSnippet: "fix it",
    });
    const result = decidePrediction(
      ...
      false,
      [],
      latestUserTurn("no tools, freeze"),
    );
    expect(result.decision).toBe("deny");
    expect(result.reason).toContain("User said: \"no tools, freeze\"");
  });

  it("latest Bash revocation denies Bash without poisoning Edit", () => {
    const pred = makeAngryLowStreak4({
      explicitlyAllowedTools: ["Edit"],
      userMessageSnippet: "fix it",
    });
    const bash = decidePrediction(
      pred,
      "Bash",
      { command: "rg -n prediction-block src" },
      4,
      "don't use Bash, fix the code",
      [],
      false,
      [],
      latestUserTurn("don't use Bash, fix the code"),
    );
    expect(bash.decision).toBe("deny");

    const edit = decidePrediction(
      pred,
      "Edit",
      { file_path: "src/foo.ts" },
      4,
